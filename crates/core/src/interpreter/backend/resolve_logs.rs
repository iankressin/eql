...
fn should_use_portal(chain: &ChainOrRpc, logs: &Logs) -> bool {
    ...
    if !logs.filter().iter().all(|f| filter_supported_by_portal(f)) {
        return false;
    }
    ...
    for filter in filters {
        match filter {
            LogFilter::EmitterAddress(addr) => {
                log_filter.insert("address".into(), json!([format!("{:?}", addr)]));
            }
...
fn parse_portal_log(
    ...
    for field in fields {
        match field {
            ...
            LogField::Topic0 => {
                result.topic0 = topics.and_then(|t| t.get(0)).and_then(value_to_b256);
            }
            LogField::Topic1 => {
                result.topic1 = topics.and_then(|t| t.get(1)).and_then(value_to_b256);
            }
            LogField::Topic2 => {
                result.topic2 = topics.and_then(|t| t.get(2)).and_then(value_to_b256);
            }
            LogField::Topic3 => {
                result.topic3 = topics.and_then(|t| t.get(3)).and_then(value_to_b256);
            }
            LogField::Data => {
...
async fn resolve_logs_via_rpc(logs: &Logs, chain_or_rpc: &ChainOrRpc) -> Result<Vec<LogQueryRes>> {
